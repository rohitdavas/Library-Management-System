...
class ViewIssued:
    ...
    def create_gui(self):
        self.GUI = GUI(page_title="Library", page_width=400, page_height = 400)
        self.GUI.create_canvas(img=None, bg="#12a4d9")
        head_frame = self.GUI.create_frame(location=(0.25, 0.1, 0.5, 0.13), bg="#FFBB00")
        self.GUI.create_label(head_frame, "Issued Books", "black", "white", (0, 0, 1, 1), 15)
        ...
        query = "SELECT * FROM " + self.issue_table
        print(query)
        try:
            cur.execute(query)
            con.commit()
